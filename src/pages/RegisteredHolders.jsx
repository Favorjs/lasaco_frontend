...
//         `${base}/api/registered-users?page=${pagination.page}&pageSize=${pagination.pageSize}&sortBy=${sortConfig.key}&sortOrder=${sortConfig.direction}&search=${searchTerm}`
//       );
//       if (!response.ok) {
...
const RegisteredHolders = () => {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const response = await fetch(
        `${base}/api/${endpoint}?page=1&pageSize=${pagination.totalItems || 1000}&sortBy=registered_at&sortOrder=desc&search=${searchTerm}`,
        {
          headers: {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `${base}/api/${endpoint}?page=${pagination.page}&pageSize=${pagination.pageSize}&sortBy=${sortConfig.key}&sortOrder=${sortConfig.direction}&search=${searchTerm}`,
        {
          headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json'
